...
class Productos(models.Model):
    ...
    codigo_barra = models.CharField(db_column='CodigoBarra', max_length=64, unique=True, null=True, blank=True)
...
class EscaneoPendiente(models.Model):
    """
    Buzón único y global donde cae el último código que la app del celular
    (QR & Barcode Scanner, POST&GET) manda por HTTP a /escaner/recibir/.

    Tanto el formulario de "agregar producto" como la pantalla de
    facturación consultan este buzón (polling, cada ~1.2s) y "toman" el
    código -> lo dejan en None- para no procesarlo dos veces. Al ser un
    solo celular físico compartido por la tienda, basta con una sola fila
    (pk=1) a modo de casillero.
    """
    id = models.AutoField(primary_key=True)
    ultimo_codigo = models.CharField(max_length=64, null=True, blank=True)
