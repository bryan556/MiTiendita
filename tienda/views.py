...
def productos_agregar_view(request):
    if request.method == 'POST':
        try:
            ...
            codigo_barra = (request.POST.get('codigo_barra') or '').strip() or None
...
def _agregar_producto_al_carrito(request, producto):
    """
    Lógica compartida para meter un producto al carrito de la sesión.
    La usan tanto el clic sobre la tarjeta del producto (id_producto)
    como el escaneo de código de barras (por lector físico o celular).
    """
    carrito = request.session.get('carrito', {})
    if not isinstance(carrito, dict):
        carrito = {}

    str_id = str(producto.id_producto)
...
def facturacion_agregar_item(request):
    if request.method == 'POST':
        ...
        _agregar_producto_al_carrito(request, producto)

    return redirect('facturacion_view')


@login_requerido
def facturacion_agregar_por_codigo(request):
    """
    Recibe el código de barra (del lector físico, o del celular vía el
    buzón de EscaneoPendiente + JS de polling) y agrega el producto
    correspondiente al carrito, igual que un clic sobre su tarjeta.
    """
    if request.method == 'POST':
        codigo = (request.POST.get('codigo_barra') or '').strip()
        if not codigo:
            messages.error(request, "No se recibió ningún código.")
            return redirect('facturacion_view')

        producto = Productos.objects.filter(codigo_barra=codigo, activo=True).first()
        if not producto:
            messages.error(request, f"⚠️ Ningún producto activo tiene el código «{codigo}». Verifica que esté registrado en Productos.")
            return redirect('facturacion_view')
...
# ========================================================
#  📱 LECTOR DE CÓDIGO DE BARRAS POR CELULAR (app externa GET/POST)
# ========================================================

def _extraer_codigo_de_peticion(request):
    """
    La app 'QR & Barcode Scanner, POST & GET' deja elegir el nombre del
    parámetro en su configuración, así que probamos varios nombres
    comunes en GET y en POST. Si no viene como key=value, probamos
    también el cuerpo plano de la petición (algunas apps mandan el
    código así, sin nombre de parámetro).
    """
    claves_posibles = ('code', 'text', 'data', 'value', 'codigo', 'barcode', 'result', 'scan')
    ...
    if request.method == 'POST' and not fuente:
        cuerpo = request.body.decode('utf-8', errors='ignore').strip()
        if cuerpo and '=' not in cuerpo:
            return cuerpo
...
@csrf_exempt
def escaner_recibir(request):
    """
    Endpoint público (sin login) al que la app del celular manda cada
    código escaneado, por GET o por POST según se configure en la app.
    No requiere login porque quien llama no es un navegador con sesión
    de Django, sino la app externa. Por eso es @csrf_exempt.
    """
    if request.method not in ('GET', 'POST'):
        return JsonResponse({'ok': False, 'error': 'Método no permitido'}, status=405)

    codigo = _extraer_codigo_de_peticion(request)
    if not codigo:
        return JsonResponse({
            'ok': False,
            'error': 'No se encontró el código en la petición. Revisa el nombre del parámetro configurado en la app.'
        }, status=400)

    buzon, _ = EscaneoPendiente.objects.get_or_create(pk=1)
    buzon.ultimo_codigo = codigo
    buzon.recibido_en = timezone.now()
    buzon.save(update_fields=['ultimo_codigo', 'recibido_en'])

    return JsonResponse({'ok': True, 'codigo': codigo})


@login_requerido
def escaner_consultar(request):
    """
    La pantalla (agregar producto o facturación) pregunta aquí cada
    ~1.2s si llegó un código nuevo. Usamos select_for_update dentro de
    una transacción para "tomar" el código y limpiarlo en un solo paso
    atómico, así nunca se procesa el mismo código dos veces.
    """
    with transaction.atomic():
        buzon = EscaneoPendiente.objects.select_for_update().filter(pk=1).first()
        codigo = None
        if buzon and buzon.ultimo_codigo:
            codigo = buzon.ultimo_codigo
            buzon.ultimo_codigo = None
            buzon.save(update_fields=['ultimo_codigo'])
