...
import pyotp
import logging
import re # Necesario para validar mayúsculas y números
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if contra and len(contra.strip()) > 0:
                # --- REGLAS DE SEGURIDAD ESTRICTAS PARA EDICIÓN ---
                if len(contra.strip()) < 8:  
                    messages.error(request, "La nueva contraseña debe tener al menos 8 caracteres.")
                    return render(request, 'usuarios/usuarios_editar.html', {'usuario': usuario_dict})
                    
                if not re.search(r"[A-Z]", contra):
                    messages.error(request, "La nueva contraseña debe contener al menos una letra mayúscula.")
                    return render(request, 'usuarios/usuarios_editar.html', {'usuario': usuario_dict})
                    
                if not re.search(r"\d", contra):
                    messages.error(request, "La nueva contraseña debe contener al menos un número.")
                    return render(request, 'usuarios/usuarios_editar.html', {'usuario': usuario_dict})
                    
                if not re.search(r"[^A-Za-z0-9]", contra):
                    messages.error(request, "La nueva contraseña debe contener al menos un signo especial (*, -, !, @...).")
                    return render(request, 'usuarios/usuarios_editar.html', {'usuario': usuario_dict})
                # --------------------------------------------------
                
                # Si pasa todas las pruebas, actualiza
...
def verificar_otp_view(request):
    if request.method == 'POST':
        ...
        if validar_otp(request, codigo):
            ...
            # ✅ CORRECCIÓN: Volvemos al nombre pelado sin el prefijo "usuarios:"
            return redirect('restablecer_password') 
        else:
            messages.error(request, "Código OTP inválido o expirado. Verifica tu aplicación.")
            # 🔥 BLOQUEO DE SEGURIDAD: Si falla, NO volvemos a generar el QR.
            return render(request, 'usuarios/otp_qr.html')

    return redirect('login')

# VISTA PARA INICIAR LA RECUPERACIÓN (PIDE USUARIO)
def recuperar_password_view(request):
    if request.method == 'POST':
        ...
        # 🔥 BLOQUEO DE SEGURIDAD: Solo mandamos a digitar el código.
        return render(request, 'usuarios/otp_qr.html') 

    return render(request, 'usuarios/recuperar_password.html')

# NUEVA VISTA PARA VINCULAR (SOLO PARA LOGUEADOS)
@login_required
def vincular_celular_otp_view(request):
    # Solo el usuario logueado puede ver su propio QR para vincular su celular
    secret = obtener_secreto_persistente_usuario(request.user)
    totp = pyotp.TOTP(secret)
    uri = totp.provisioning_uri(name=request.user.username, issuer_name="MiTiendita")
    
    return render(request, 'usuarios/vincular_qr.html', {'uri': uri})
...
def restablecer_password_view(request):
    ...
    if request.method == 'POST':
        ...
        if not re.search(r"\d", nueva_pass):
        ...
        try:
            ...
            # 2. Hacemos el inicio de sesión automático
            auth_login(request, user)

            # 3. Mantenemos compatibilidad con las variables globales
            request.session['user_id'] = user.id
            request.session['user_nombre'] = user.username
            request.session['user_role_compatible'] = 'Admin' if user.is_superuser else 'Vendedor' 
            request.session['user_rol'] = 'Admin' if user.is_superuser else 'Vendedor'
